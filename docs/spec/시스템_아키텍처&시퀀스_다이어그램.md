...
- **나머지가 레이어드인 이유**: Member, Chat, Review, Report는 외부 의존이 적고 비즈니스 로직이 단순(CRUD + 알파 수준)합니다. 포트 인터페이스/구현체 분리에 따른 보일러플레이트 대비 이점이 부족하므로 레이어드 구조로 유지합니다.
- **Notification이 포트/어댑터인 이유**: SSE 푸시(`NotificationPushPort` → `NotificationSseAdapter`), JPA 접근 추상화(`NotificationRepository`, `PriceAlertRepository`)로 외부 시스템 의존이 있어 포트/어댑터 패턴을 적용합니다.
...
| `AuctionLifecyclePort` | `RedisAuctionLifecycle` | Redis TTL 기반 경매 상태 전환 추상화 |
| `AuctionEventPublisher` | `KafkaAuctionEventPublisher` | Kafka 경매 이벤트 발행 추상화 |
...
| `AutoBidRepository` | `AutoBidRepositoryImpl` • `AutoBidJpaRepository` | 자동 입찰 DB 접근 추상화 |
| `BidEventPublisher` | `KafkaBidEventPublisher` | Kafka 입찰 이벤트 발행 추상화 |
| `AuctionLockPort` | `RedissonAuctionLock` | Redisson 분산 락 추상화 |

### Notification 도메인

| 포트 인터페이스 (biddo-domain) | 구현체 (biddo-infra) | 역할 |
| --- | --- | --- |
| `NotificationRepository` | `NotificationRepositoryImpl` • `NotificationJpaRepository` | DB 접근 추상화 |
| `PriceAlertRepository` | `PriceAlertRepositoryImpl` • `PriceAlertJpaRepository` | 가격 알림 DB 접근 추상화 |
| `NotificationPushPort` | `NotificationSseAdapter` | SSE 푸시 추상화 |

### Search 도메인 (부분 포트)

| 포트 인터페이스 (biddo-domain) | 구현체 (biddo-infra) | 역할 |
| --- | --- | --- |
| `AuctionSearchPort` | `AuctionSearchAdapter` | ES 검색 + CircuitBreaker + DB fallback |
| `RecentSearchPort` | `RecentSearchAdapter` | Redis List 기반 최근 검색어 |
...
[Bid Service] → bid-events 토픽 → [ES Sync Consumer] → price/bidCount 업데이트
                                  → [Popular Auction Consumer] → Redis Sorted Set 인기도 갱신
```
...
- EC2 자체 설치 Redis에서는 `redis.conf`에서 `notify-keyspace-events Ex` 설정 필요.
...
# 8. 이미지 처리: S3 직접 업로드 & CloudFront CDN

## 결정 사항

클라이언트가 **Presigned URL로 S3에 직접 업로드**하고, **CloudFront를 통해 이미지를 서빙**합니다.

## 처리 플로우

1. 클라이언트가 API 서버에 Presigned URL 발급 요청 (`POST /api/v1/upload/presigned`)
2. API 서버가 S3 SDK로 Presigned PUT URL 생성 후 반환 (만료: 10분)
   - 함께 CloudFront 기반 접근 URL (`fileUrl`) 반환
3. 클라이언트가 Presigned URL로 S3에 원본 이미지 직접 PUT 업로드
4. 이미지 접근 시 CloudFront CDN을 통해 캐싱하여 서빙

## 경로 구조

| 용도 | S3 경로 |
| --- | --- |
| 경매 이미지 | `auctions/{uuid}_{filename}` |
| 채팅 이미지 | `chat/{uuid}_{filename}` |
| 프로필 이미지 | `profiles/{uuid}_{filename}` |
...
# 10. Self-hosted 인프라 전환

## 결정 사항

AWS Managed Service(RDS, ElastiCache, MSK, OpenSearch)를 **EC2 직접 설치**로 전환합니다.

## 선택 이유

- **비용 절감**: MSK 최소 월 $150+, ElastiCache $15+, OpenSearch $25+, RDS $15+ → EC2 직접 설치 시 인스턴스 비용만 발생
- **포트폴리오 프로젝트 특성**: 고가용성(Multi-AZ, 자동 복구)보다 비용 효율이 우선
- **코드 변경 불필요**: 표준 프로토콜 클라이언트(Spring Kafka, Spring Data Redis, JDBC, Spring Data Elasticsearch)를 사용하므로 접속 주소 변경만으로 전환 가능

## 코드 영향도

| 영역 | 변경 사항 |
| --- | --- |
| Java 코드 | 없음 — 표준 클라이언트가 프로토콜 수준에서 호환 |
| `application.yml` | `DB_HOST`, `REDIS_HOST` 등 환경 변수 기본값만 사용, 실제 값은 `.env`에서 주입 |
| `docker-compose.yml` | `env_file: .env`로 환경 변수 통일 |
| `.env` / `.env.example` | 인프라 접속 정보 관리 (`.env`는 gitignored) |
| GitHub Secrets | `DB_PASSWORD`, `JWT_SECRET` 등 민감 값 저장 → CI/CD에서 `.env` 생성 |

## Self-hosted 시 운영 고려사항

| 항목 | Managed Service | Self-hosted | 대응 방안 |
| --- | --- | --- | --- |
| 자동 백업 | RDS 자동 스냅샷 | 없음 | `pg_dump` 크론잡 설정 |
| 자동 복구 | Multi-AZ 장애 조치 | 없음 | 수동 복구 (포트폴리오 수준에서 허용) |
| 패치/업그레이드 | 자동 마이너 업데이트 | 수동 | Docker 이미지 태그 관리 |
| 모니터링 | CloudWatch 통합 | 별도 설정 | Prometheus + Grafana로 커버 |
| Redis Keyspace Notification | ElastiCache 파라미터 그룹 | `redis.conf` 직접 설정 | `notify-keyspace-events Ex` 설정 필수 |
| Kafka 토픽 관리 | MSK 콘솔 | 수동 생성 | `KAFKA_AUTO_CREATE_TOPICS_ENABLE: false` → 배포 스크립트에서 토픽 생성 |

## 환경 변수 관리 전략

```
.env.example   ← Git 커밋 (템플릿, 기본값)
.env           ← .gitignore (로컬/서버 각각 실제 값)
GitHub Secrets ← DB_PASSWORD, JWT_SECRET 등 민감 값
```

- **로컬**: `.env`에 localhost 기본값, docker-compose가 자동 로드
- **EC2 서버**: CI/CD가 GitHub Secrets + 인프라 주소로 `.env` 생성 후 배포